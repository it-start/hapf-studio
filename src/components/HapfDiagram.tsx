...
import { Box, FileInput, Settings } from 'lucide-react';
import { detectProviderFromText, getProviderTheme } from '../theme/providers';
...
const HapfDiagram: React.FC<HapfDiagramProps> = ({ code, activeModule }) => {
  ...
  const parseHapf = useCallback(() => {
    ...
    while (cursor < code.length) {
        ...
        const providerType = detectProviderFromText(blockContent);
        if (providerType) {
            const theme = getProviderTheme(providerType);
            providerIcon = <theme.Icon size={12} className={`${theme.colorClass} absolute top-2 right-2`} />;
        }
            ...
            if (rtBlockStart !== -1) {
                ...
                if (rtBlockEnd !== -1) {
                    ...
                    const rtConfig = lines.slice(0, 3).map((l, i) => {
                        const detectedProvider = detectProviderFromText(l);
                        let icon = null;
                        if (detectedProvider && l.includes('model')) {
                            const theme = getProviderTheme(detectedProvider);
                            icon = <theme.Icon size={10} className={theme.colorClass} />;
                        }
                    ...
                    const runtimeProvider = detectProviderFromText(rtContent);
                    if (runtimeProvider) {
                        const theme = getProviderTheme(runtimeProvider);
                        customRuntimeStyle.borderColor = theme.borderColor;
                    }
