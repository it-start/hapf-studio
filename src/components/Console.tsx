...
import { getProviderTheme, getLogCategory } from '../theme/providers';
...
const LogIcon = ({ level, isThought, message }: { level: LogLevel, isThought: boolean, message: string }) => {
  ...
  if (category) {
      const { Icon, colorClass } = category;
      return <Icon className={`w-3 h-3 ${colorClass} mt-0.5`} />;
  }
...
const ProviderBadge = ({ provider }: { provider?: AIProvider }) => {
    ...
    const { Icon, colorClass, bgClass, borderClass, label } = theme;

    return (
        <span className={`flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded border ${bgClass} ${colorClass} ${borderClass} font-bold mr-2`}>
            <Icon size={8} /> {label}
        </span>
    );
};
