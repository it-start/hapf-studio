...
/**
 * Generic helper to execute a Gemini request and return parsed JSON.
 * Reduces boilerplate for standard structural generation tasks.
 */
async function generateStructured<T>(
  ...
  try {
    ...
    const defaultFallback = responseSchema?.type === Type.ARRAY ? "[]" : "{}";
    return JSON.parse(response.text || defaultFallback);
  } catch (error) {
    console.error("Gemini Generation Error:", error);
    // Return a safe empty object/array based on schema type if possible, or empty object
    return (responseSchema?.type === Type.ARRAY ? [] : {}) as T;
  }
}
...
export const runIngestFiles = async (jsonInput: string): Promise<VirtualFile[]> => {
  return generateStructured<VirtualFile[]>(
    `Parse the following JSON input into a list of VirtualFile objects.\n<json_input>\n${jsonInput}\n</json_input>`,
    "You are a Virtual File System parser. Extract the file list from the 'virtual_files' key in the JSON.",
    {
      type: Type.ARRAY,
      items: {
        type: Type.OBJECT,
...
export const runAnalyzeArchitecture = async (files: VirtualFile[]): Promise<ProjectArchitecture> => {
  return generateStructured<ProjectArchitecture>(
    `Analyze these virtual files to determine the project architecture: ${JSON.stringify(files)}`,
    "Analyze the file hints. Extract dependencies (e.g., from package.json hints), state keys (from store hints), and the framework.",
    {
      type: Type.OBJECT,
      properties: {
        dependencies: { type: Type.ARRAY, items: { type: Type.STRING } },
        store_keys: { type: Type.ARRAY, items: { type: Type.STRING } },
...
export const runGenerateSpec = async (arch: ProjectArchitecture): Promise<GeneratedSpec> => {
  return generateStructured<GeneratedSpec>(
    `Generate a HAPF v1.0 specification for a project with this architecture: ${JSON.stringify(arch)}.`,
    "Generate a valid HAPF v1.0 code block describing this architecture. Include modules for the main components.",
    {
      type: Type.OBJECT,
      properties: {
 ...
 * (Kept separate from helper as it requires usage metadata retrieval)
 */
export const runGenericPipelineSimulation = async (
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      ...
      4. **Search Simulation**:
         - If a module uses 'google_search', simulate a list of realistic search results (titles, URLs, snippets) relevant to the query in the 'data_preview' or 'message'.

      5. Simulate realistic output for each module based on the input context.
      6. If there are conditional branches (if/else), evaluate them based on your simulated data.
...
export const runCompileToN8n = async (hapfCode: string, instanceUrl: string): Promise<N8nWorkflowData> => {
  return generateStructured<N8nWorkflowData>(
    `Compile this HAPF code into a VALID n8n Workflow JSON structure.\n\nHAPF CODE:\n${hapfCode}\n\nTarget Instance URL: ${instanceUrl || "http://localhost:5678"}`,
    `You are a HAPF to n8n Compiler.
